...
class QueryLogger:
    ...
    def __init__(self, log_file="/data/query_logs.csv"):
...
query_logger = QueryLogger("query_logs.csv")
...
def extract_filename_from_url(url):
    ...
    return unquote(filename.split('?')[0])  # Remove query params
...
async def run_enhanced_query(request: ClaimRequest, token: str = Depends(verify_token)):
    ...
    document_path = None
    if document_links:
        filename = extract_filename_from_url(document_links[0])
        document_path = os.path.join("db", os.path.splitext(filename)[0])  # e.g., /db/principia_newton
    ...
    print("should", should_skip)
    # Detect document type for logging
    document_type = "unknown"
    
    try:
        # Step 1: Process documents only if not already processed
        if should_skip and vector_store is None and document_path and os.path.exists(os.path.join(document_path, "index.faiss")):
            try:
                print(f"💾 Loading vector store from {document_path}")
                vector_store = FAISS.load_local(document_path, embeddings,allow_dangerous_deserialization=True)
                processed_documents = vector_store.docstore._dict.values()
                    ...
                    for i in range(0, len(chunks), batch_size):
                        ...
                        try:
                            ...
                            if i == 0:
                                ...
                                if document_path:
                                    try:
                                        vector_store.save_local(document_path)
                                        print(f"💾 Vector store saved to: {document_path}")
                                    except Exception as e:
...
class SimpleDocumentTracker:
    ...
    def __init__(self):
        ...
        self.known_document_links = {
            ...
            "https://hackrx.blob.core.windows.net/assets/UNI GROUP HEALTH INSURANCE POLICY - UIIHLGP26043V022526 1.pdf",
            "https://hackrx.blob.core.windows.net/assets/Family Medicare Policy (UIN- UIIHLIP22070V042122) 1.pdf",
            "https://hackrx.blob.core.windows.net/assets/Super_Splendor_(Feb_2023).pdf",
            "https://hackrx.blob.core.windows.net/assets/Arogya Sanjeevani Policy - CIN - U10200WB1906GOI001713 1.pdf"
        }
    ...
    def normalize_url(self, url: str) -> str:
        ...
        return urlunparse((parsed.scheme, parsed.netloc, decoded_path, '', '', ''))
